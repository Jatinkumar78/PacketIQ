...
_NON_HOST_IPS = {UNSPECIFIED_IPV4, UNSPECIFIED_IPV6}
...
class DataExtractor:
    ...
    def feed(self, record: RawPacketRecord):
        ...
        if record.eth_src and record.eth_src not in ("ff:ff:ff:ff:ff:ff",):
            ...
            if src_ip and src_ip != UNSPECIFIED_IPV4:
        ...
        if record.has_http:
            if record.http_method:
                r.http_requests.append({
                    ...
                    "src":    record.src_ip,
                    "dst":    record.dst_ip,
                    # The server port this actually went to. HTTP is recognised on
                    # any port now, so consumers must not assume 80.
                ...
                r.http_responses.append({
                    ...
                    "src":    record.src_ip,
                    "dst":    record.dst_ip,
                    # Responses are sourced from the server, so its port is src_port.
    ...
    def _feed_arp(self, record: RawPacketRecord, ts: float):
        ...
        if sip and sip != UNSPECIFIED_IPV4 and smac and smac not in self._NULL_MACS:
        ...
        if sip and sip != UNSPECIFIED_IPV4 and tip and tip != sip:
    ...
    def _canonical_ip(ips: set, ip_mac_counts: dict, mac: str) -> Optional[str]:
        ...
        def _rank(ip: str) -> tuple:
            is_v6 = ":" in ip
            v4_ll = (not is_v6) and (ip.startswith("169.254.") or ip == UNSPECIFIED_IPV4)
