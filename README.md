...
![Tests](https://img.shields.io/badge/tests-1745%20passing-22c55e?style=for-the-badge&logo=pytest&logoColor=white)
...
**`100%` recall** · **`90%` precision** on real CTU-13 malware · **`15`** detection types · **`7,600+`** live threat-intel indicators · **`1,745`** tests · **`ruff`-clean**
...
| Trust | unverifiable claims | **1,745 tests, 100% line coverage, CI, ruff-clean, fuzz-tested parser** |
...
pytest                                                # run the 1,745-test suite
...
Coverage here is a property of the *suite*, not of the machine that runs it, and that distinction is enforced rather than assumed. Three rules are denied outright in `tests/conftest.py`: a test may not open a socket, may not open a real capture device, and never sees the machine's own list of network interfaces. Without those guards a workstation quietly flatters the number — a Mac with capture privileges really does start a live capture, a machine running `ollama serve` really does find an AI provider, and a laptop with four Thunderbolt bridge adapters really does exercise the branch that ranks them. Every OS-specific and host-specific path — the macOS/Linux/Windows capture-privilege checks, the interface enumerator, the `tomllib`/`tomli` split, the whole `/api/live/*` lifecycle — is therefore driven from a fixture, so a line is covered on every matrix leg or on none. Verified by measuring the suite under simulated runner environments (Linux dispatch, denied capture socket, no local model, no external tools) on 3.9, 3.11 and 3.12: 100.00% in every one.
...
```bash
pip install -e ".[dev]"
pytest                 # 1,745 tests: parser, detectors, enrichment, TLS, SIGMA, export, fuzz, AI, security
